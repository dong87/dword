#' Conflicts between the dword and other packages
#'
#' This function lists all the conflicts between packages in the dword
...
#' @examples
#' dword_conflicts()
dword_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  tidy_names <- paste0("package:", dword_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "dword_conflicts")
}

dword_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "dword_conflicts()"
  )
...
#' @export
print.dword_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(dword_conflict_message(x))
}
